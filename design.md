...
> individual block to `(2**16)-1` bytes (65,535 bytes, approximately 64 KiB). This provides a balance between
> efficiency and protection against memory-exhaustion attacks.
...
point of divergence. Combined with the ideal-permutation assumption on $f$, this means distinct transcripts produce
independent, pseudorandom outputs. The proof thus anchors the framework's CMT-4 context commitment: every output bit is
cryptographically bound to the full history of operations that preceded it, with no two histories collapsing to the same
permutation input sequence.
