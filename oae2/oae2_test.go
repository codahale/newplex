...
import (
	...
	"github.com/codahale/newplex/oae2"
)
...
func TestNewReader(t *testing.T) {
	t.Run("invalid block size", func(t *testing.T) {
		...
		oae2.NewReader(newplex.NewProtocol("test"), nil, 0)
	})

	t.Run("multiple blocks", func(t *testing.T) {
		pWriter := newplex.NewProtocol("test")
	...
	t.Run("reordered blocks", func(t *testing.T) {
		...
		data := buf.Bytes()
		blockLen := blockSize + newplex.TagSize
	...
	t.Run("truncated stream missing bytes", func(t *testing.T) {
		...
		_, err := io.ReadAll(r)
		if !errors.Is(err, newplex.ErrInvalidCiphertext) {
			t.Fatalf("expected newplex.ErrInvalidCiphertext, got %v", err)
		}
	})
...
func TestWriter_Write(t *testing.T) {
	t.Run("underlying writer error", func(t *testing.T) {
		...
		w := oae2.NewWriter(newplex.NewProtocol("example"), ew, 64)
...
func TestWriter_Close(t *testing.T) {
	t.Run("idempotent close", func(t *testing.T) {
		var buf bytes.Buffer
		w := oae2.NewWriter(newplex.NewProtocol("example"), &buf, 64)
...
func TestReader_Read(t *testing.T) {
	t.Run("empty read", func(t *testing.T) {
		r := oae2.NewReader(newplex.NewProtocol("example"), bytes.NewReader(nil), 64)
		n, err := r.Read(nil)
		if n != 0 || err != nil {
	...
	t.Run("underlying reader error", func(t *testing.T) {
		...
		r := oae2.NewReader(newplex.NewProtocol("example"), er, 64)
...
func TestNewWriter(t *testing.T) {
	t.Run("invalid block size", func(t *testing.T) {
		...
		oae2.NewWriter(newplex.NewProtocol("test"), nil, 0)
	})
}
...
func Example() {
	encrypt := func(key, plaintext []byte) []byte {
		...
		p := newplex.NewProtocol("com.example.oae2")
	...
	// Output:
	// ciphertext = c1887a820f3f02203292ae196696759816a8c023dc70214a90094ede592ee883adff9df1d27021fbd169dc45c6a0541db731f401a4c18749174a805fa73c2939ab1146d0d169e1a4dfc1ae67c2e4d821
...
func FuzzReader(f *testing.F) {
	drbg := testdata.New("newplex oae2 fuzz")
	for range 10 {
		f.Add(drbg.Data(1024))
	}

	f.Fuzz(func(t *testing.T, data []byte) {
		r := oae2.NewReader(newplex.NewProtocol("fuzz"), bytes.NewReader(data), 64)
		v, err := io.ReadAll(r)
		if err == nil {
